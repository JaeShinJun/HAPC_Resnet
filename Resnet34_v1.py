...
def predict_submission(model, submission_load):
    all_preds = []
    # model.eval()
    for i, b in tqdm(enumerate(submission_load, 0), total=int(len(subset)/submission_load.batch_size)) :
        X, labels = b
...
def make_submission_file(sample_submission_df, predictions):
    ...
    sample_submission_df.to_csv('submission_ver6.csv', index=None)
...
trainloader = DataLoader(trainset, collate_fn=trainset.collate_func, batch_size=128, num_workers=2)
validationloader = DataLoader(validationset, collate_fn=validationset.collate_func, batch_size=32, num_workers=2)
# submissionloader = DataLoader(subset, collate_fn=subset.collate_func, batch_size=18, num_workers=2)
...
def test():
    ...
    # net.load_state_dict(torch.load('./resnet34_v1.pth'))
    # net.load_state_dict(torch.load('./MVL_resnet34_v1.pth'))
    ...
    minimum_validation_loss = 99999999.0
    for epoch in range(1000) :
        ...
        #     torch.save(net.state_dict(), './inception2.pth')

        net.eval()
        validation_loss = 0.0
        with torch.no_grad():
            for i, data in tqdm(enumerate(validationloader, 0), total=int(len(validationset)/validationloader.batch_size)) :
                inputs, labels = data
        ...
        if(validation_loss < minimum_validation_loss) :
            ...
            torch.save(net.state_dict(), './MVL_resnet34_v1.pth')


        torch.save(net.state_dict(), './resnet34_v1.pth')
