...
# df_train = df
df_train, df_validation = train_test_split(df, test_size=0.1, random_state=SEED)
...
class SampleNet(nn.Module):
    def __init__(self):
        ...
        self.model = torchvision.models.inception_v3(pretrained=True, aux_logits=True)
        self.model.fc = nn.Linear(2048, 28)
        self.model.aux_logits = False
        
    def forward(self, x):
        x = x[:, :3, ...].nn.BatchNorm2d(3)
        return self.model(x)
...
def evaluate(model, optimizer, criterion) :
    ...
    with torch.no_grad():
        ...
        f1 = f1_score(labels, predicted>0.1, average='macro')

        if(validation_loss < minimum_validation_loss) :
            minimum_validation_loss = validation_loss
            torch.save(model.state_dict(), './MVL_Inception_v4.pth')
    return validation_loss, f1

def scoring(model) :
    model.eval()
    max_data = [0., 0.]
    for THRESHOLD in [0.1 + (0.01*i) for i in range(30)] :
        all_preds = []
        true = []
...
def test():
    ...
    # net = load_parameter(net, './MVL_Inception_v4.pth')
    # net = load_parameter(net, './Inception_v4.pth')
